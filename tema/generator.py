...
class Publication:
    def __init__(self, car_model: str, production_date: datetime.datetime, horsepower: int, color: str, max_speed: int):
...
class FieldConfig:
    def __init__(
        self, field_name="CAR", value_generator_function=lambda: 0, containing_ratio=0.25, operators_frequencies=None
    ):

        # frecvente default pentru operatori
        if not operators_frequencies:
            operators_frequencies = {"==": 0.5, "!=": 0, ">=": 0, ">": 0, "<=": 0, "<": 0}
...
class SubscriptionsGenerator:
    def __init__(self, subscriptions_count=100, configs=None):
    ...
    def generate(self):
        ...
        for config in self.configs:
            ...
            subscriptions_to_edit = random.sample(empty_subscriptions, to_pick_from_empty) + random.sample(
                non_empty_subscriptions, to_pick_from_non_empty
            )
            ...
            for i in subscriptions_to_edit:
                ...
                subscriptions[i][config.field_name] = {"operator": operator, "value": config.generate_value()}

        for s in subscriptions:
            if not s:
                config = random.choice(self.configs)
                operator = random.choice([op for op, frequency in config.operators_frequencies.items() if frequency])
                s[config.field_name] = {"operator": operator, "value": config.generate_value()}
...
def dump_subscriptions():
    subscriptions = SubscriptionsGenerator(
        100,
        [
            FieldConfig(
                field_name="car_model",
    ...
    with open("subscriptions.txt", "w") as handle:
        for subscription in subscriptions:
            ...
            for attribute in subscription:
                ...
                res = res + "(" + attribute + "," + subscription[attribute]["operator"] + "," + val + ");"
